...
export type CommunitySubCommentsFormProps = {
  id: string,
  comments?: Array<any>,
  parentCommentId: any,
...
export default function CommunitySubCommentsForm({ id, comments, parentCommentId, subCommentFormInputRef, onSetComments, onSetChildCommentCount }: CommunitySubCommentsFormProps) {
  ...
  const handleSubmitForm = (e: React.FormEvent<HTMLFormElement>): void  => {
    ...
    if (!comment.trim()) {
      ...
      const url = `/api/community/comment/?communityId=${id}&parentCommentId=${parentCommentId}&contents=${comment}`;
  ...
  return (
    <form onSubmit={handleSubmitForm} css={communityCommentsFormStyle}>
      <div>
        <input type="text" name="contents" id="contents" max="2000" value={comment} onChange={handleCommentData} ref={subCommentFormInputRef} />
      </div>
      <div>
